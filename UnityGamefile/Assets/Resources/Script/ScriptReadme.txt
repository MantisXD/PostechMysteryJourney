...
//주석은 그 자체로 한줄을 차지해야 합니다. 다른 스크립트 옆에 주석을 붙이지 마세요.
...
//NPC의 번호(1부터 시작하거나, 공차가 1인 등차수열일 필요는 없습니다)와 좌표를 다음과 같이 입력해주세요
//각 NPC가 어떤 Image를 사용하는지는 NPC의 번호로 결정됩니다.
//좌표는 왼쪽 위 점에서 각 NPC의 정 중앙 까지의 거리로 나타냅니다.
//여기서 '거리'는 맵 전체 크기의 (n/100)배를 나타내는 비례상수로, 0부터 100까지의 정수 값을 가집니다(가령, 맵의 정 중앙에 놓여질 NPC는 x축 좌표와 y축 좌표를 둘 다 50으로 해주세요
//맵의 오른쪽 끝에 온다고 해서 둘 다 100으로 하진 마세요
NPC 1 [x축 좌표] [y축 좌표]
...
//나레이션이 없으면 NarrStart와 NarrEnd 사이에 NULL을 적어주세요
...
//Script가 하나 뿐이라서 순서를 고려할 필요가 없는 경우에도 순서는 1을 입력해주세요
